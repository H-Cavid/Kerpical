      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-10">
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-3xl md:text-5xl font-black text-white mb-3 tracking-tighter leading-tight uppercase italic">{product.name}</h1>
              <div className="w-16 h-1.5 bg-green-500 rounded-full shadow-[0_0_15px_rgba(34,197,94,0.5)]"></div>
            </div>
            <p className="text-slate-400 text-base md:text-lg leading-relaxed font-medium">{product.desc}</p>
            ...
            <div className="space-y-4">
              <a href={`https://wa.me/994776235836?text=${encodeURIComponent(product.name + " " + t.wp)}`} onClick={() => trackWhatsAppClick("Product Page")} className="w-full bg-green-600 hover:bg-green-500 text-white py-5 rounded-2xl flex items-center justify-center gap-3 font-black text-lg transition-all shadow-xl hover:shadow-green-500/20 active:scale-95 border-b-4 border-green-700 hover:border-green-600">
                <MessageCircle className="w-6 h-6" /> {t.order}
              </a>
            </div>
            <button onClick={scrollToDetails} className="w-full flex items-center justify-center gap-2 text-slate-500 hover:text-white transition-colors pt-4 text-sm font-bold group">
              <span>{t.more}</span>
        ...
        <div id="details" className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-white/5 scroll-mt-24">
          <div className="space-y-6">
            <h3 className="text-xl font-black flex items-center gap-3 text-white uppercase tracking-wider italic"><Zap className="text-green-500 w-5 h-5 fill-green-500" /> {t.why}</h3>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-black flex items-center gap-3 text-white uppercase tracking-wider italic"><Layers className="text-green-500 w-5 h-5" /> {t.usage}</h3>
            <div className="bg-green-600/5 border border-green-500/10 p-6 rounded-3xl space-y-4">
              {product.areas?.map((text, i) => (
                <div key={i} className="flex items-center gap-3 group">
                  <div className="w-8 h-8 rounded-full bg-green-500/10 flex items-center justify-center text-green-500 text-xs font-bold border border-green-500/20">{i + 1}</div>
